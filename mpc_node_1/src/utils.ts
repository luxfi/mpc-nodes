...
const signClientName = process.env.sign_client_name + process.env.node_number
...
const killSigner = async (signerProc: string) => {
  try {
    ...
    let cmd = "killall -9 " + signerProc.toString()
          ...
          if (upStdout) {
            ...
            if (Number(upStdoutArr[upStdoutArr.length - 1]) + Number(upStdoutArr[upStdoutArr.length - 2]) * 60 >= smTimeOutBound) {
              ...
              try {
                ...
                const outKill = await exec(cmd, { cwd: __dirname + "/multiparty" }) // Make sure it"s dead
              } catch (err) {
